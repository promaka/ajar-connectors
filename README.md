...
```bash
git clone --depth 1 --branch v0.6.2 https://github.com/promaka/ajar-connectors
```
...
| Language | Command |
|---|---|
| Rust | `ajar-connector = "0.6.2"` |
| Python | `pip install ajar-connector==0.6.2` |
...
docker run \
  ...
  ghcr.io/promaka/ajar-connector-asterix:0.6.2 /etc/ajar/connector.toml
```
...
| [feed-synthetic](rust/connectors/feed-synthetic) | — (**conformance source**) | one fixture on six real wire formats, for building against and commissioning before equipment arrives | multicast / TCP / UDP |
...
Pin the released tag `v0.6.2`, not a branch.
