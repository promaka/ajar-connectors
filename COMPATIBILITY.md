...
1. **Pin a released version, not a branch.** Depend on an exact release
   (`0.5.11`, or the tag `v0.5.11` where the install goes through git), never
   `branch = "main"` — `main` moves. With a pinned release, even your *next
   rebuild* is reproducible.
   - Rust: `ajar-connector = "0.5.11"` (crates.io)
   - Go: `go get github.com/promaka/ajar-connectors/go/ajarconnector@v0.5.11`
   - Python: `pip install ajar-connector==0.5.11` (PyPI)
   - C++: check out the `v0.5.11` tag (or vendor it) and build per [cpp/README.md](cpp/README.md).
